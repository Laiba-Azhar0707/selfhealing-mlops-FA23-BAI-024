# Self-Healing MLOps Pipeline — FA23-BAI-024

**Student:** Laiba Azhar · **Roll No:** FA23-BAI-024
...
## Assigned customization values (row FA23-BAI-024)

| Field | Value |
|---|---|
| Confidence Threshold | `0.593` (`alert.rules.yml` expr + Grafana red threshold line) |
| Stable Model Code | `stable-v0-57F7` (`stable-fallback/app.py` `model_version`, both occurrences) |
| Webhook Token | `ROLLBACK_B49942_TOKEN` (`alertmanager.yml` + Jenkins rollback trigger) |
| Test Category / Text | `APP` — "This app is incredibly intuitive and has made my daily workflow dramatically more efficient" |
...
alert.rules.yml             # alert ModelConfidenceDrift, expr < 0.593 for 1m
alertmanager.yml            # webhook -> Jenkins rollback (token ROLLBACK_B49942_TOKEN)
k8s/
...
stable-fallback/            # build context for the stable image (also a dedicated branch)
  app.py                    # rule-based stable fallback (model_version stable-v0-57F7)
  requirements.txt
  Dockerfile
```

The **`stable-fallback` branch** holds the stable `app.py` (with code `57F7`),
...
exporter.py :8000 ──poll /api/latest-confidence──► (app) ◄──── socat host:32500 ────────┘
        ...
        ▼
Prometheus :9090 ──(alert ModelConfidenceDrift < 0.593 for 1m)──► Alertmanager :9093
                                                                        │ webhook (token)
                                                                        ▼
...
Grafana :3000  ── dashboard "MLOps - Sentiment API Health" (red threshold line @ 0.593)
```
