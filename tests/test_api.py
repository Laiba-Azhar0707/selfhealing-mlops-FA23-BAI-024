...
def test_predict_returns_label_and_confidence():
    ...
    Uses the assigned APP test sentence (row FA23-BAI-024)."""
    r = requests.post(f"{BASE_URL}/predict", json={"text": "This app is incredibly intuitive and has made my daily workflow dramatically more efficient"})
    assert r.status_code == 200
    data = r.json()
